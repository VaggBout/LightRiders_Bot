...
class Board:
    ...
    def is_legal(self, row, col, my_id) -> bool:
        enemy_id = (my_id + 1) % 2
    ...
    def is_legal_tuple(self, loc, my_id):
        row, col = loc
        return self.is_legal(row, col, my_id)

    def get_adjacent(self, row: int, col: int, player_id, use_stored=True) -> List:
        if use_stored and (row, col) in self.adjacents:
            ...
            for (o_row, o_col), _ in DIRS:
                ...
                if self.is_legal(t_row, t_col, player_id):
